      ...
      {board.map((row, i) => (
        <div key={`row-${i}`} className="flex gap-1">
          {row.map((cell, j) => (
            <button
              key={`pos-${j}-${i}`}
              className={`border-2 border-gray-900 w-10 h-10 cursor-pointer items-center justify-center text-2xl font-bold flex ${isWinningCell(i, j) ? "bg-green-600 animate-pulse-fast" : ""}`}
